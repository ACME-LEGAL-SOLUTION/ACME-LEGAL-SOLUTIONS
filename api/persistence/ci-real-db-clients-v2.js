...
function mysqlClient({ config }) {
  ...
  const pool = mysql.createPool({ uri: config.url, waitForConnections: true });
  return { connect: () => pool.getConnection(), close: () => pool.end(), async query(sql, params) { const [rows] = await pool.query(sql, params); return { rows: Array.isArray(rows) ? rows : [] }; } };
}

function mariadbClient({ config }) {
  const mariadb = require("mariadb");
  const pool = mariadb.createPool({ uri: config.url, connectionLimit: 5 });
  return { connect: () => pool.getConnection(), close: () => pool.end(), async query(sql, params) { const connection = await pool.getConnection(); try { const rows = await connection.query(sql, params); return { rows: Array.isArray(rows) ? rows : [] }; } finally { connection.release(); } } };
}
...
async function createSqliteLock({ config }) {
  ...
  return {
    ...
    async acquire() { if (handle) throw new Error("Migration lock already held"); handle = await fs.open(lockPath, "wx"); },
