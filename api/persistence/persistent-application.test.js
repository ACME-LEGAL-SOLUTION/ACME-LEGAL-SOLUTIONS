...
function transactionProvider(events, txExecutor) { return {
  begin: async () => ({ id: "tx-1", query: txExecutor.query.bind(txExecutor) }),
  commit: async (tx) => events.push(["commit", tx.id]), rollback: async (tx) => events.push(["rollback", tx.id])
}; }

test("persistent application composes the full runtime over SQL repositories", async () => {
  const executor = executorFromRows(); const events = [];
  const app = createPersistentApplication({ executor, provider: testProvider, ...transactionProvider(events, executor) });
  assert.ok(app.application.crm); assert.ok(app.application.intake); assert.ok(app.application.document); assert.ok(app.application.evidence);
  assert.ok(app.repositories.clients); assert.ok(app.crmOperations); assert.ok(app.matterDomainOperations);
  ...
  assert.equal(client.id, "client-1"); assert.match(executor.calls[0].sql, /^INSERT INTO clients/);
  assert.deepEqual(executor.calls[0].params.slice(0, 3), ["client-1", "individual", "prospective"]); assert.equal((await app.repositories.clients.getById("client-1")).id, "client-1"); assert.equal(events.length, 0);
});

test("persistent client-matter operation binds both writes to the transaction executor", async () => {
  const calls = []; const executor = executorFromRows(calls); const events = [];
  const app = createPersistentApplication({ executor, provider: testProvider, ...transactionProvider(events, executor) });
  ...
  assert.equal(result.matter.clientId, "client-2"); assert.deepEqual(events, [["commit", "tx-1"]]);
});
...
test("persistent CRM composition exposes the transaction-scoped client-matter operation", async () => {
  ...
  assert.equal(result.matter.clientId, "client-3"); assert.deepEqual(events, [["commit", "tx-1"]]);
});

test("persistent matter authorization denies before transaction and preserves action", async () => {
  const events = []; const calls = []; const executor = executorFromRows(calls); const decisions = [];
  const app = createPersistentApplication({ executor, provider: testProvider, ...transactionProvider(events, executor), resolveMatterAccess: async (input) => { decisions.push(input); return input.actor.id === "actor-1" && input.matterId === "matter-1" && input.action === "update"; } });
  ...
  assert.deepEqual(events, []); assert.deepEqual(calls, []); assert.deepEqual(decisions[0], { actor: { id: "actor-2" }, matterId: "matter-1", action: "update" });
  assert.equal(await app.matterOperations.run({ actor: { id: "actor-1" }, matterId: "matter-1", action: "update", work: async ({ tx }) => tx.id }), "tx-1");
