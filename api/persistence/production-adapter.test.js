...
test("production adapter requires an explicit production transaction or complete driver", () => { assert.throws(() => createProductionAdapter({ config: config(), repositories: repositories() }), /requires connect/); });
test("production adapter wires the complete PostgreSQL driver contract", () => {
  ...
  assert.equal(adapter.provider, "postgresql"); assert.equal(adapter.driver.provider, "postgresql"); assert.equal(adapter.dialect.bind("SELECT ?", ["x"]).sql, "SELECT $1");
  assert.equal(typeof adapter.connect, "function"); assert.equal(typeof adapter.close, "function"); assert.equal(typeof adapter.query, "function"); assert.equal(typeof adapter.transaction.run, "function"); assert.equal(typeof adapter.ensureMigrationLedger, "function");
});
test("driver-backed transaction commits on success and rolls back on failure", async () => {
  const events = []; const dbDriver = { ...driver(), begin: async () => ({ id: "tx-1" }), commit: async (tx) => events.push(["commit", tx.id]), rollback: async (tx) => events.push(["rollback", tx.id]) };
...
test("production adapter preserves question-mark binding for MySQL", () => { const adapter = createProductionAdapter({ config: config("mysql"), repositories: repositories(), driver: driver("mysql") }); assert.equal(adapter.dialect.bind("SELECT ?", ["x"]).sql, "SELECT ?"); });
