...
function createProductionMessagingProvider({ env = process.env, provider = null } = {}) {
  const resolvedProvider = provider || (() => {
    ...
    if (!name) throw new Error("Production messaging provider adapter is not configured (ACME_MESSAGING_PROVIDER_MODULE)");
    return require(name);
  })();

  if (!resolvedProvider || typeof resolvedProvider.send !== "function" || typeof resolvedProvider.verifyWebhook !== "function") {
    throw new TypeError("Messaging provider must expose send and verifyWebhook");
  }

  return Object.freeze({
    id: text(resolvedProvider.id || env.ACME_MESSAGING_PROVIDER_ID || "messaging-provider", "Messaging provider id"),
    async send(input = {}) {
      if (!ALLOWED_CHANNELS.has(input.channel)) throw new TypeError("Unsupported messaging channel");
      const body = text(input.body, "body");
      if (body.length > MAX_BODY) throw new TypeError("Message body is too large");
    ...
    async verifyWebhook(input = {}) {
      const signature = input.signature || input.headers?.["x-message-signature"] || input.headers?.["X-Message-Signature"];
...
function createMessagingGovernance({
  ...
  if (!messageRepository?.create || !messageRepository?.getById || !messageRepository?.list || !messageRepository?.update) {
  ...
  if (retryMax < retryBase) throw new TypeError("retryMaxMs must be greater than or equal to retryBaseMs");

  async function authorize(actor, matterId) {
    if (!actor?.id) throw new Error("Authenticated actor is required");
    if (matterId) {
      if (!matterAuthorization?.assert) throw new Error("Matter authorization is required");
      await matterAuthorization.assert(actor, matterId, "update");
    }
  }
  ...
  async function enqueue(input = {}) {
    if (!ALLOWED_CHANNELS.has(input.channel)) throw new TypeError("Unsupported messaging channel");
    if (!input.id || !input.idempotencyKey) throw new TypeError("Message id and idempotencyKey are required");
    text(input.recipientRef, "recipientRef");
    await authorize(input.actor, input.matterId);

    const duplicate = await messageRepository.list({ idempotencyKey: input.idempotencyKey });
    if (duplicate.length) return { duplicate: true, message: duplicate[0] };
    ...
    await writeAudit({ message, actor: input.actor, eventType: "messaging.queued", payload: { messageId: input.id, channel: input.channel }, now });
    return { duplicate: false, message };
  }

  async function applyTransition({ id, status, actor, providerMessageId = null, failureReason = null, skipAuthorization = false } = {}) {
    const message = await messageRepository.getById(id);
    if (!message) throw new Error("Message not found");
    if (!skipAuthorization) await authorize(actor, message.matterId);
    ...
    if (status === "failed") {
      ...
      changes.nextRetryAt = attempts < attemptsLimit
        ? new Date(clock().getTime() + retryDelayMs(attempts, retryBase, retryMax)).toISOString()
        : null;
    }
    if (status === "queued") {
      changes.failedAt = null;
      changes.failureReason = null;
      changes.nextRetryAt = null;
    }
  ...
  async function retry({ id, actor } = {}) {
    ...
    if (Number(message.attemptCount || 0) >= attemptsLimit) throw new Error("Message retry limit exhausted");
    if (message.nextRetryAt && new Date(message.nextRetryAt).getTime() > clock().getTime()) throw new Error("Message retry is not due yet");
    return applyTransition({ id, status: "queued", actor });
  }

  async function deliver({ id, actor, deliveryProvider = provider } = {}) {
    if (!deliveryProvider?.send) throw new Error("Messaging delivery provider is required");
    const message = await messageRepository.getById(id);
    if (!message) throw new Error("Message not found");
    await authorize(actor, message.matterId);
    if (message.status !== "queued") throw new Error(`Only queued messages can be delivered (current: ${message.status})`);

    const sending = await applyTransition({ id, status: "sending", actor });
    try {
      ...
      return applyTransition({ id, status: "sent", actor, providerMessageId: result?.providerMessageId || result?.id || null });
    } catch (error) {
      const reason = error instanceof Error ? error.message : "delivery failed";
      return applyTransition({ id, status: "failed", actor, failureReason: reason });
    }
  }

  async function handleWebhook({ input, actor = { id: "system-messaging", type: "system" }, deliveryProvider = provider } = {}) {
    if (!deliveryProvider?.verifyWebhook) throw new Error("Messaging webhook verifier is required");
    const verified = await deliveryProvider.verifyWebhook(input || {});
    if (!verified || verified.verified !== true) throw new Error("Messaging webhook verification failed");

    const providerMessageId = text(verified.providerMessageId || verified.messageId, "providerMessageId");
    const status = text(verified.status, "status");
    if (!["delivered", "failed"].includes(status)) throw new Error("Unsupported verified webhook status");
    ...
    // Provider signature verification is the authorization boundary for this
    // external callback. Matter authorization is intentionally not invoked with
    // a fabricated human actor.
