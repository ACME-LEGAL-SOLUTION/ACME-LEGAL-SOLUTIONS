...
function createApplicationBootstrap({ env = process.env, clock = () => new Date(), provider = null, authenticate = null, clientFactories = productionClients, sqliteLockFactory = productionClients.createSqliteLock, productionPersistenceFactory = createProductionPersistence, applicationRuntimeFactory = createApplicationRuntime, migrationManifest = manifest, rootDir = process.cwd() } = {}) {
  const production = env.ACME_ENV === "production";
  async function start() {
    if (!production) {
      const repositories = createApplicationRepositories({ clock });
      return Object.freeze({ environment: env.ACME_ENV || "development", application: applicationRuntimeFactory({ repositories, provider: provider || DEVELOPMENT_AI_PROVIDER, clock }), repositories, persistence: null, authenticate, async close() {} });
    }
    ...
    try {
      ...
      return Object.freeze({ environment: "production", application, repositories: persistence.repositories, persistence, migrationRunner, authenticate: productionAuthenticator, async close() { await persistence.driver.close(); } });
    } catch (error) {
