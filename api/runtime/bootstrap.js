...
function createApplicationBootstrap({ env = process.env, clock = () => new Date(), provider = null, authenticate = null, matterAuthorization = null, clientFactories = productionClients, sqliteLockFactory = productionClients.createSqliteLock, productionPersistenceFactory = createProductionPersistence, applicationRuntimeFactory = createApplicationRuntime, migrationManifest = manifest, rootDir = process.cwd() } = {}) {
  ...
  async function start() {
    ...
    try {
      ...
      const application = applicationRuntimeFactory({ repositories: persistence.repositories, provider, clock, matterAuthorization: productionMatterAuthorization });
      return Object.freeze({ environment: "production", application, repositories: persistence.repositories, persistence, migrationRunner, authenticate: productionAuthenticator, matterAuthorization: productionMatterAuthorization, async close() { await persistence.driver.close(); } });
    } catch (error) {
