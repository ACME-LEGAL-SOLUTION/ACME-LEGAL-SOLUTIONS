...
const { createProductionObjectStorage } = require("../security/production-object-storage");
...
function createApplicationBootstrap({ env = process.env, clock = () => new Date(), provider = null, authenticate = null, matterAuthorization = null, objectStorage = null, clientFactories = productionClients, sqliteLockFactory = productionClients.createSqliteLock, productionPersistenceFactory = createProductionPersistence, applicationRuntimeFactory = createApplicationRuntime, migrationManifest = manifest, rootDir = process.cwd(), aiProviderFactory = createProductionAIProvider, objectStorageFactory = createProductionObjectStorage } = {}) {
  const production = env.ACME_ENV === "production";
  async function start() {
    if (!production) {
      const repositories = createApplicationRepositories({ clock });
      return Object.freeze({ environment: env.ACME_ENV || "development", application: applicationRuntimeFactory({ repositories, provider: provider || DEVELOPMENT_AI_PROVIDER, clock, matterAuthorization, objectStorage }), repositories, persistence: null, authenticate, matterAuthorization, objectStorage, async close() {} });
    }
    ...
    const productionObjectStorage = objectStorage || objectStorageFactory({ env });
    ...
    try {
      ...
      const application = applicationRuntimeFactory({ repositories: persistence.repositories, provider: productionProvider, clock, matterAuthorization: productionMatterAuthorization, objectStorage: productionObjectStorage });
      return Object.freeze({ environment: "production", application, repositories: persistence.repositories, persistence, migrationRunner, authenticate: productionAuthenticator, matterAuthorization: productionMatterAuthorization, objectStorage: productionObjectStorage, provider: productionProvider, async close() { await persistence.driver.close(); } });
    } catch (error) {
