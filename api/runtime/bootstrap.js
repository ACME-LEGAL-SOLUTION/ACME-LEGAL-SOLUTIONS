...
function createApplicationBootstrap({ env = process.env, clock = () => new Date(), provider = null, clientFactories = productionClients, sqliteLockFactory = productionClients.createSqliteLock, productionPersistenceFactory = createProductionPersistence, migrationManifest = manifest, rootDir = process.cwd() } = {}) {
  const production = env.ACME_ENV === "production";
  async function start() {
    if (!production) {
      const repositories = createApplicationRepositories({ clock });
      return Object.freeze({ environment: env.ACME_ENV || "development", application: createApplicationRuntime({ repositories, provider: provider || DEVELOPMENT_AI_PROVIDER, clock }), repositories, persistence: null, async close() {} });
    }
    ...
    try {
      ...
      const application = createApplicationRuntime({ repositories: persistence.repositories, provider, clock });
