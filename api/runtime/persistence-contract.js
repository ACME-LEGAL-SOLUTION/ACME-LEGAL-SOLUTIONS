...
function assertRepositoryContract(repositories) {
  ...
  for (const name of COLLECTIONS) {
    ...
    for (const method of METHODS) if (typeof repository[method] !== "function") throw new Error(`Repository ${name}.${method} is required`);
  }
...
function createRepositoryAdapter({ repositories }) { assertRepositoryContract(repositories); return Object.freeze({ repositories, assert: () => assertRepositoryContract(repositories) }); }
