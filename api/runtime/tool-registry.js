...
function strings(value, field) {
  if (!Array.isArray(value) || value.length === 0 || value.some((v) => typeof v !== "string" || !v)) throw new TypeError(`${field} must be a non-empty array of strings`);
...
function normalizeTool(input) {
  ...
  strings(input.humanApproval.actions.length ? input.humanApproval.actions : ["none"], "humanApproval.actions");
  ...
  const copy = { ...input, humanApproval: { ...input.humanApproval }, limits: { ...input.limits }, execute: input.execute };
  return freeze(copy);
}
