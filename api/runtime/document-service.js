...
function createDocumentService({ repositories, clock = () => new Date(), objectStorage = null, matterAuthorization = null } = {}) {
  ...
  async function createDocument(input, actor) {
    ...
    return repositories.documents.create({ matterId: input.matterId, name: input.name.trim(), type: input.type, status: input.status || "received", source: input.source || "client", storageKey: input.storageKey || null, mimeType: input.mimeType || null, checksum: input.checksum || null, metadata: input.metadata && typeof input.metadata === "object" ? input.metadata : {}, createdBy: actor.id, createdAt: clock().toISOString() }, actor);
  }
  ...
  async function putObject({ matterId, objectId, body, contentType, checksum }, actor) {
    requireActor(actor); requireStorage(); await assertMatterAccess(matterId, actor, "update");
    return objectStorage.put({ matterId, objectId, body, contentType, checksum });
  }
  async function getObject({ matterId, objectId }, actor) {
    requireActor(actor); requireStorage(); await assertMatterAccess(matterId, actor, "read");
    return objectStorage.get({ matterId, objectId });
  }
  async function deleteObject({ matterId, objectId }, actor) {
    requireActor(actor); requireStorage(); await assertMatterAccess(matterId, actor, "update");
    return objectStorage.delete({ matterId, objectId });
  }
